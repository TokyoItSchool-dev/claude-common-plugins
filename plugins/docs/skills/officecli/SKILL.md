...
`officecli` requires installing and running a native CLI binary. In a sandboxed environment (e.g. Claude Code on the web) that disallows installing or executing arbitrary binaries, installation may fail or the binary may be unavailable. If so, say that officecli cannot run in this environment and stop rather than improvising a workaround.

## Origin

user skills directory (officecli)
